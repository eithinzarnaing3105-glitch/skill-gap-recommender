...
Respond with ONLY a valid JSON array, no markdown code fences, no extra commentary. Each item must be an
object with exactly two keys: "skill" (a short skill name, 1-4 words) and "reason" (one plain-language
sentence, no jargon, tied to the patterns above). Example format:

[{{"skill": "Python", "reason": "Job postings pairing SQL with programming almost always ask for Python too."}}]
"""
    return prompt


def parse_recommendation_json(raw_text):
    """Strip any accidental code fences and parse the JSON array Gemini returns."""
...
def get_recommendation(current_skills, target_role, rules_df, top_n=8, max_retries=3):
    ...
    if relevant.empty:
        return (
            "No matching skill patterns were found for these inputs. "
            "Try different or more general current skills.",
            relevant,
        )
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                skills = parse_recommendation_json(response.text)
